...
def test_private_candidates_exclude_out_of_project_source_roots(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    """A source root outside project_root is read-only reference context, not scanned output.

    Regression test for findings leaking from sibling projects listed only as
    reference ``source_roots``.
    """
    ...
    symbols = _symbols(libmain)
    assert symbols == {("util.helpers", "unused_public_helper")}

    assert cli_main([str(libmain)]) == 1
    output = capsys.readouterr().out
    assert "Found 1 public symbol that could be made private:" in output
    assert "python/util/helpers.py:1: function `unused_public_helper`" in output
    assert "consumer.py" not in output
    assert "another_public_symbol" not in output


def test_method_candidates_exclude_out_of_project_source_roots(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    """A method candidate whose class lives in a sibling's source root is not reported."""
    core = tmp_path / "core"
    sibling = tmp_path / "sibling"
    _write(
        sibling / "service.py",
...
""".strip()
    ...
    assert "class `AppService` (1 of 2 public methods)" in output
    assert "class `Service`" not in output
    assert sibling.resolve().as_posix() not in output


def test_private_module_imports_exclude_out_of_project_source_roots(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    """A private-module import from a sibling's out-of-project source root is not reported.

    The finding is scoped by ``imported_by_path`` (the importing file), not the
    imported module, since the defect belongs to whoever wrote the bad import.
    """
    core = tmp_path / "core"
    sibling = tmp_path / "sibling"
    _write(
    ...
    assert "src/pkg/two/public.py" in output
    assert sibling.resolve().as_posix() not in output


def test_private_symbol_imports_exclude_out_of_project_source_roots(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    """A private-symbol import from a sibling's out-of-project source root is not reported."""
    core = tmp_path / "core"
    sibling = tmp_path / "sibling"
    _write(
    ...
    assert "src/pkg/consumer.py" in output
    assert "imports private symbol `pkg.producer._PrivateService`" in output
    assert sibling.resolve().as_posix() not in output


def test_export_issues_exclude_out_of_project_source_roots(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    """An __all__ export issue in a sibling's out-of-project source root is not reported."""
    core = tmp_path / "core"
    sibling = tmp_path / "sibling"
    _write(core / "src" / "exports.py", '__all__ = ["MISSING"]\n')
    _write(sibling / "sibling_exports.py", '__all__ = ["MISSING"]\n')
    _write(
        core / "tach.toml",
        'source_roots = ["src", "../sibling"]\n',
    )
    ...
    assert "src/exports.py" in output
    assert "__all__ exports unknown name `MISSING`" in output
    assert sibling.resolve().as_posix() not in output


def test_unparsable_modules_exclude_out_of_project_source_roots(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    """An unparsable file in a sibling's out-of-project source root is not reported."""
    core = tmp_path / "core"
    sibling = tmp_path / "sibling"
    _write(core / "src" / "broken.py", "def oops(:\n    pass\n")
    _write(sibling / "sibling_broken.py", "def oops(:\n    pass\n")
    _write(
        core / "tach.toml",
        'source_roots = ["src", "../sibling"]\n',
    )
