...
```bash
uv build
uv publish dist/test_privata-0.2.0-py3-none-any.whl dist/test_privata-0.2.0.tar.gz
```

Pass the specific filenames rather than running `uv publish` bare. Bare `uv publish` uploads everything in `dist/`, which may include stale or dirty builds from earlier runs and will cause a 400 error from PyPI.
...
**Avoiding dirty builds:** `hatch-vcs` appends a date suffix (e.g. `+g5c6b0bc9d.d20260613`) when the working tree has uncommitted changes, producing a local version identifier that PyPI rejects. Always run `uv build` from a clean tree (`git status` shows nothing) on an exact tagged commit.
